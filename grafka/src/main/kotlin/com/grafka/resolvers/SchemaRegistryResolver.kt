...
class SchemaRegistryResolver(private val clientFactory: SchemaRegistryClientFactory) : GraphQLQueryResolver {
    ...
    fun schemaRegistrySubjects(clusterId: String) = getClient(clusterId)
            .allSubjects
            .map { KafkaSchemaRegistrySubject(clusterId, it, this) }
    fun schemaMetadata(clusterId: String, subject: String): KafkaSchemaRegistrySubjectMetadata {
        val metadata = getClient(clusterId).getLatestSchemaMetadata(subject)
        return KafkaSchemaRegistrySubjectMetadata(metadata.id, metadata.version, metadata.schema)
    }
}
